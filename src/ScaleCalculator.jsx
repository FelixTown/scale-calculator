...
// ─── Shared sub-components (outside main to prevent remount on render) ────────
    ...
    >
      <div className="px-6 pb-2 pt-5 md:px-8 md:pt-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold uppercase tracking-[0.22em]" style={{ color: palette.textMuted }}>
              {title}
            </div>
            {subtitle ? (
              <div className="mt-1 text-sm" style={{ color: palette.textSoft }}>
                {subtitle}
              </div>
            ) : null}
          </div>
...
function InputRow({ label, value, onChange, suffix, prefix = '', note, palette, isDark, optional }) {
  ...
  return (
    <div
      className="grid grid-cols-1 gap-3 py-6 md:grid-cols-[1fr_190px] md:items-center"
      style={{ borderBottom: `1px solid ${palette.borderSoft}` }}
    >
      <div>
        <div className="flex items-center gap-3">
          <div className="text-[22px] font-medium tracking-[-0.02em] md:text-[24px]" style={{ color: palette.text }}>
            {label}
          </div>
          {optional && (
            <span
              ...
              style={{
                background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(5,5,5,0.05)',
        ...
        {note ? (
          <div className="mt-2 max-w-2xl text-sm leading-6" style={{ color: palette.textSoft }}>
            {note}
          </div>
        ) : null}
      </div>
      <div className="relative">
        {prefix ? (
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-medium" style={{ color: palette.textSoft }}>
            {prefix}
          </span>
        ) : null}
        <input
        ...
        />
        {suffix ? (
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-base font-medium" style={{ color: palette.textSoft }}>
            {suffix}
          </span>
        ) : null}
      </div>
...
function BigStatCard({ label, value, sublabel, color = ACCENT, palette, isDark }) {
  return (
    <div
    ...
    >
      ...
      <div className="mt-4 text-4xl font-bold tracking-[-0.04em]" style={{ color }}>{value}</div>
...
export default function ScaleCalculator() {
  ...
  // Inputs – Improvement Assumptions
  ...
  const verdictLevel = returnMultiple === null ? 'neutral'
    : returnMultiple < 2 ? 'red'
    : returnMultiple < 3 ? 'yellow'
    : 'green'
  ...
  const verdictMessage = (() => {
    if (verdictLevel === 'neutral') return 'Enter your numbers above to see your ROI.'
    if (verdictLevel === 'red') return "You shouldn't do this. Keep what you're doing."
    if (verdictLevel === 'yellow') return "Same-ish money, but we remove the admin headache."
    return `You make ${currency(ltgp, 0)} for every ${cacNew !== null ? currency(cacNew, 0) : '—'} you put in. This is a no-brainer.`
  })()

  // ── Formatters ──
  const currency = (v, d = 0) =>
    new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: d, minimumFractionDigits: d })
      .format(isFinite(v) ? v : 0)

  const pct = (v, d = 1) =>
    `${isFinite(v) ? v.toFixed(d) : '0'}%`

  const num = (v, d = 0) =>
    new Intl.NumberFormat('en-US', { maximumFractionDigits: d, minimumFractionDigits: d })
      .format(isFinite(v) ? v : 0)
  ...
  return (
    <div className="min-h-screen" style={{ background: `${palette.pageGlow}, ${palette.bg}` }}>
      <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-10">
        ...
        {/* ── Hero ── */}
        <div
          className="mb-8 rounded-[34px] p-6 md:p-8"
          style={{
            background: palette.hero,
            border: `1px solid ${palette.border}`,
            boxShadow: `${palette.glow}, inset 0 1px 0 rgba(255,255,255,${isDark ? '0.05' : '0.85'})`,
          }}
        >
          <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
            <div className="max-w-3xl">
              <div
                className="inline-flex items-center rounded-full px-4 py-2 text-xs font-bold uppercase tracking-[0.24em]"
                style={{
                  background: isDark ? 'rgba(111,230,0,0.10)' : 'rgba(111,230,0,0.12)',
                  border: `1px solid ${isDark ? 'rgba(111,230,0,0.22)' : 'rgba(111,230,0,0.24)'}`,
                  color: isDark ? '#C9FF9F' : '#4D9B00',
                }}
              >
                Scale Automotive
              </div>
            ...
            <button
              ...
              style={{
                ...
                border: `1px solid ${palette.border}`,
                color: palette.text,
                boxShadow: isDark ? 'inset 0 1px 0 rgba(255,255,255,0.04)' : 'inset 0 1px 0 rgba(255,255,255,0.92)',
              }}
            >
        ...
        <div className="space-y-6">
          ...
          <Section title="Business Economics" subtitle="Your shop's core unit economics" {...p}>
            <InputRow {...p} label="Average job value" prefix="$"
              value={avgJobValue} onChange={(v) => setAvgJobValue(clamp(v, 1))}
              note="Blended average across tint, PPF, detailing, and any other services." />
            <InputRow {...p} label="Gross margin on an average job" suffix="%"
              value={grossMargin} onChange={(v) => setGrossMargin(clamp(v, 1, 99))}
              note="Revenue minus direct job costs (materials, labor). Not net profit." />
            <InputRow {...p} label="Repeat jobs per customer per year" suffix="×"
              value={repeatJobsPerYear} onChange={(v) => setRepeatJobsPerYear(clamp(v, 0.1))}
              note="How many times does the average customer come back in a 12-month window?" />
          </Section>

          {/* ── Section 2: Current Funnel ── */}
          <Section title="Your Current Funnel" subtitle="Where you are right now" accentColor="#FFC84A" {...p}>
            <InputRow {...p} label="Leads per month" value={leadsPerMonth}
              onChange={(v) => setLeadsPerMonth(clamp(v, 0))}
              note="All inbound leads across every source — Google, Instagram, referrals, walk-ins." />
            <InputRow {...p} label="Lead-to-job close rate" suffix="%"
              value={closeRate} onChange={(v) => setCloseRate(clamp(v, 0, 100))}
              note="What % of leads actually become paying jobs?" />
            <InputRow {...p} label="Monthly ad spend" prefix="$"
              value={adSpend} onChange={(v) => setAdSpend(clamp(v, 0))}
              note="Set to 0 if you don't run paid ads." />
            <InputRow {...p} label="Website conversion rate" suffix="%" optional
              value={websiteConvRate} onChange={(v) => setWebsiteConvRate(clamp(v, 0, 100))}
              note="% of website visitors who contact you. Leave at 0 if unknown." />
          </Section>

          {/* ── Section 3: Your Pricing ── */}
          <Section title="Scale Automotive Pricing" subtitle="What you're charging them" accentColor="#6BF6FF" {...p}>
            <InputRow {...p} label="Setup fee" prefix="$"
              value={setupFee} onChange={(v) => setSetupFee(clamp(v, 0))} />
            <InputRow {...p} label="Monthly fee" prefix="$"
              value={monthlyFee} onChange={(v) => setMonthlyFee(clamp(v, 0))} />
          </Section>

          {/* ── Section 4: Improvement Assumptions ── */}
          <Section title="Improvement Assumptions" subtitle="Conservative estimates — you control these on the call" accentColor="#FFC84A" {...p}>
            <InputRow {...p} label="Expected lead lift from better site + reviews" suffix="%"
              value={leadLift} onChange={(v) => setLeadLift(clamp(v, 0, 500))}
              note="How many more leads from the same traffic? A better site + review system typically drives 20–40%." />
            <InputRow {...p} label="Expected close rate lift from better follow-up" suffix="%"
              value={closeLift} onChange={(v) => setCloseLift(clamp(v, 0, 200))}
              note="A CRM with automated follow-up typically closes 15–25% more leads. Be conservative." />
          </Section>

          {/* ── Section 5: Per-Customer Economics ── */}
          <Section title="Per-Customer Economics" subtitle="What each customer is actually worth to you" {...p}>
            <MetricRow {...p} label="Gross profit per job" value={currency(gpPerJob, 0)} />
            <MetricRow {...p} label="Lifetime gross profit (LTGP) per customer"
              value={currency(ltgp, 0)}
              valueClassName="text-[#6FE600]"
              sublabel={`${currency(gpPerJob, 0)} × ${repeatJobsPerYear} jobs/yr`} />
          </Section>

          {/* ── Section 6: Current Performance ── */}
          <Section title="Your Current Monthly Performance" accentColor="#FFC84A" {...p}>
            <MetricRow {...p} label="Customers per month" value={num(custCurrent, 1)} />
            <MetricRow {...p} label="Monthly gross profit (new customers)" value={currency(gpMonthCurrent, 0)} />
            {cacCurrent !== null ? (
              <MetricRow {...p} label="Customer acquisition cost (CAC)" value={currency(cacCurrent, 0)} />
            ) : (
              <MetricRow {...p} label="Customer acquisition cost (CAC)" value="—" sublabel="No ad spend entered" />
            )}
            {ratioCurrent !== null ? (
              <MetricRow {...p} label="LTGP:CAC ratio"
                value={`${ratioCurrent.toFixed(1)}x`}
                valueClassName={ratioCurrent >= 3 ? 'text-[#6FE600]' : ratioCurrent >= 2 ? (isDark ? 'text-yellow-300' : 'text-yellow-700') : (isDark ? 'text-red-400' : 'text-red-700')} />
            ) : (
              <MetricRow {...p} label="LTGP:CAC ratio" value="—" sublabel="Enter ad spend to calculate" />
            )}
          </Section>

          {/* ── Section 7: With Scale Automotive ── */}
          <Section title="With Scale Automotive" subtitle="What your funnel looks like after we go live" accentColor="#6BF6FF" {...p}>
            <MetricRow {...p} label="New leads per month"
              value={num(leadsNew, 1)}
              sublabel={`+${leadLift}% lift from ${num(leadsPerMonth, 0)} current`} />
            <MetricRow {...p} label="New close rate"
              value={pct(closeRateNew * 100, 1)}
              sublabel={`+${closeLift}% lift from ${pct(closeRate, 0)} current`} />
            <MetricRow {...p} label="Customers per month"
              value={num(custNew, 1)}
              valueClassName="text-[#6FE600]" />
            <MetricRow {...p} label="Monthly gross profit (new customers)"
              value={currency(gpMonthNew, 0)}
              valueClassName="text-[#6FE600]" />
            <MetricRow {...p} label="Extra customers per month because of us"
              value={`+${num(extraCustomers, 1)}`}
              valueClassName="text-[#6FE600]" />
            <MetricRow {...p} label="Extra gross profit per month because of us"
              value={`+${currency(deltaGPMonth, 0)}`}
              valueClassName="text-[#6FE600]" />
            {cacNew !== null ? (
              <MetricRow {...p} label="New CAC (including our fee)"
                value={currency(cacNew, 0)}
                sublabel="Ad spend + monthly fee, divided by new customers" />
            ) : null}
            {ratioNew !== null ? (
          ...
          <Section title="The Verdict" accentColor="#6BF6FF" {...p}>
            ...
            {/* Color bar */}
            <div
              className="rounded-[26px] px-6 py-8 md:px-10 md:py-10 mb-6"
              style={{ background: vc.bg, border: `1px solid ${vc.border}` }}
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <div className="text-sm font-bold uppercase tracking-[0.2em] mb-2" style={{ color: vc.text, opacity: 0.7 }}>
                    {vc.label}
                  </div>
                  <div className="text-xl font-medium leading-8 max-w-xl" style={{ color: vc.text }}>
                    {verdictMessage}
                  </div>
                </div>
                {returnMultiple !== null && (
                  <div className="text-center shrink-0">
                    <div className="text-6xl font-bold tracking-[-0.06em] md:text-7xl" style={{ color: vc.text }}>
                      {returnMultiple.toFixed(1)}x
                    </div>
                    <div className="text-sm font-semibold uppercase tracking-[0.15em] mt-1" style={{ color: vc.text, opacity: 0.65 }}>
                      return multiple
                    </div>
                  </div>
                )}
              ...
              <div className="mt-8">
                <div className="h-2.5 rounded-full overflow-hidden" style={{ background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(5,5,5,0.08)' }}>
                  <div
                    ...
                    style={{
                      ...
                      background: verdictLevel === 'green'
                        ? `linear-gradient(90deg, rgba(111,230,0,0.6), ${accent})`
                        : verdictLevel === 'yellow'
                        ? 'linear-gradient(90deg, rgba(250,204,21,0.6), #FACC15)'
                        : 'linear-gradient(90deg, rgba(251,113,133,0.6), #FB7185)',
                    }}
                  />
                </div>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[10px] font-bold uppercase tracking-[0.12em]">
                  <div className="rounded-full py-1" style={{ background: isDark ? 'rgba(136,19,55,0.22)' : 'rgba(251,113,133,0.10)', color: isDark ? '#FDA4AF' : '#BE123C' }}>
                    &lt; 2x — Don't do it
                  </div>
                  <div className="rounded-full py-1" style={{ background: isDark ? 'rgba(120,92,22,0.18)' : 'rgba(250,204,21,0.12)', color: isDark ? '#FDE68A' : '#A16207' }}>
                    2–3x — Borderline
                  </div>
                  <div className="rounded-full py-1" style={{ background: isDark ? 'rgba(111,230,0,0.10)' : 'rgba(111,230,0,0.10)', color: isDark ? accent : '#4D9B00' }}>
                    &gt; 3x — No-brainer
                  </div>
                </div>
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <BigStatCard {...p}
                label="Extra profit per month"
                value={deltaGPMonth > 0 ? `+${currency(deltaGPMonth, 0)}` : '—'}
                sublabel="vs. your current baseline"
                color={accent}
              />
              <BigStatCard {...p}
                label="Break-even"
                value={paybackWeeks !== null && paybackWeeks > 0 ? `${paybackWeeks.toFixed(1)} wks` : '—'}
                sublabel={paybackMonths !== null && paybackMonths > 0 ? `${paybackMonths.toFixed(1)} months` : 'Enter numbers above'}
                color={isDark ? '#6BF6FF' : '#0891B2'}
              />
              <BigStatCard {...p}
                label="Annual extra take-home"
                value={annualExtraProfit > 0 ? `+${currency(annualExtraProfit, 0)}` : '—'}
                sublabel="extra gross profit per year"
                color={accent}
              />
              <BigStatCard {...p}
                label="For every $1 in ads + system"
                value={returnMultiple !== null ? `$${returnMultiple.toFixed(2)}` : '—'}
                sublabel="in lifetime gross profit"
                color={isDark ? '#6BF6FF' : '#0891B2'}
              />
            </div>
